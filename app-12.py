...
def fetch_deriv_candles(symbol: str, granularity: int, count: int = CANDLE_COUNT) -> tuple[Optional[pd.DataFrame], Optional[str]]:
    """Pull OHLC candles from Deriv's public WebSocket API.

    Returns (df, None) on success or (None, reason) on failure so the caller
    can fall back to synthetic data *and* tell the user why the live feed
    wasn't used, instead of silently faking it.
    """
    if not WEBSOCKET_AVAILABLE:
        return None, "The 'websocket-client' package isn't installed."

    last_error = None
    for attempt in range(2):  # one retry — Deriv occasionally drops the first handshake
        try:
            ...
            if "error" in response:
                last_error = response["error"].get("message", "Unknown API error")
                continue

            candles = response.get("candles", [])
            if not candles:
                last_error = "Deriv returned an empty candle set."
                continue
...
def generate_synthetic_data(symbol: str, granularity: int, count: int = CANDLE_COUNT) -> pd.DataFrame:
    """DEMO-ONLY fallback feed, used solely when the live Deriv socket can't
    be reached, so the dashboard remains interactive.

    IMPORTANT: real Volatility Index levels are the output of an
    ever-running, non-mean-reverting random walk that Deriv has operated
    for years — there is no fixed "true" price to anchor to, and the actual
    level today can only come from Deriv's live feed. The anchors below are
    rough, order-of-magnitude placeholders (approximate recent trading
    ranges) purely so the chart looks plausible in demo mode; they are NOT
    real quotes and will drift out of date. Treat any signal generated on
    synthetic data as illustrative only, never as a real trading signal.
    """
    # (annualised volatility %, approximate recent order-of-magnitude level)
    # Levels below were taken directly from a live Deriv quotes screen
    # (Sept 2026) for the symbols that were visible there. R_50 / 1HZ50V
    # were NOT visible in that screen, so those two remain rough guesses —
    # treat any signal on those two specifically as unverified until the
    # live feed connects.
    profile_map = {
        "R_10": (0.10, 4_950),        # confirmed: ~4949.79 / 4950.20
        "R_25": (0.25, 2_570),        # confirmed: ~2570.03 / 2570.90
        "R_50": (0.50, 1_800),        # UNCONFIRMED placeholder
        "R_75": (0.75, 45_380),       # confirmed: ~45364.97 / 45397.75
        "R_100": (1.00, 590),         # confirmed: ~590.20 / 590.67
        "1HZ10V": (0.10, 9_722),      # confirmed: ~9721.75 / 9722.59
        "1HZ25V": (0.25, 895_000),    # confirmed: ~894889.48 / 895043.47
        "1HZ50V": (0.50, 12_000),     # UNCONFIRMED placeholder
        "1HZ75V": (0.75, 5_445),      # confirmed: ~5443.05 / 5447.24
        "1HZ100V": (1.00, 1_018),     # confirmed: ~1017.48 / 1018.19
    }
    sigma, base_price = profile_map.get(symbol, (0.5, 1_000))
    seed = abs(hash((symbol, granularity, int(time.time() // 60)))) % (2 ** 32)
    rng = np.random.default_rng(seed)

    dt = granularity / 86400  # fraction of a day per bar
    returns = rng.normal(loc=0.0, scale=sigma * np.sqrt(dt) * 0.02, size=count)
    close = base_price * np.exp(np.cumsum(returns) - np.cumsum(returns)[0])
...
def get_market_data(symbol: str, granularity: int) -> tuple[pd.DataFrame, bool, Optional[str]]:
    """Returns (dataframe, is_live, connection_error) with a short TTL cache."""
    df, error = fetch_deriv_candles(symbol, granularity)
    if df is not None and len(df) > EMA_PERIOD:
        return df, True, None
    return generate_synthetic_data(symbol, granularity), False, error
...
def calc_ema(series: pd.Series, length: int) -> pd.Series:
    """Exponential Moving Average (standard EWM, no external TA library needed)."""
    return series.ewm(span=length, adjust=False).mean()


def calc_rsi(series: pd.Series, length: int) -> pd.Series:
    """Wilder's RSI. With length=1 (alpha=1/1=1) the smoothing term collapses
    to the current bar only, reproducing the unsmoothed, single-candle RSI
    called for by the strategy spec (snaps toward 0/100 each impulse candle)."""
    delta = series.diff()
    ...
    rsi = 100 - (100 / (1 + rs))
    # All-gain bars (avg_loss == 0) => RSI saturates at 100; all-loss => 0.
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["rsi1"] = calc_rsi(df["close"], RSI_PERIOD)
...
raw_df, is_live, connection_error = get_market_data(symbol_code, granularity)
...
if not is_live:
    with st.container():
        st.warning(
            "⚠️ **Demo mode — not real prices.** The live Deriv feed could not be reached, "
            "so the chart below uses a simulated placeholder series for demonstration only. "
            "Signals, entries, SL/TP shown while in this mode are illustrative — do not trade them.",
            icon="⚠️",
        )
        if connection_error:
            with st.expander("Connection diagnostics"):
                ...
                st.caption(
                    "Common causes: outbound WebSocket (wss://) traffic blocked by your host/firewall, "
                    "`websocket-client` not installed, or a temporary Deriv API hiccup. "
...
c4.metric("Data Feed", "🟢 Live (Deriv)" if is_live else "🟡 Demo (not real prices)")
if signal:
